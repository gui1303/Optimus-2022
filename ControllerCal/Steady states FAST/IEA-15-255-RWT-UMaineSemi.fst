...
--------------------------- Input Files ---------------------------------------- 
"IEA-15-255-RWT-UMaineSemi_ElastoDyn.dat" EDFile      - Name of file containing ElastoDyn input parameters (quoted string) 
...
"IEA-15-255-RWT_UMaineSemi_InflowFile.dat" InflowFile - Name of file containing inflow wind input parameters (quoted string) 
"AeroData/IEA-15-255-RWT_AeroDyn15.dat" AeroFile      - Name of file containing aerodynamic input parameters (quoted string) 
"IEA-15-255-RWT-UMaineSemi_ServoDyn.dat" ServoFile    - Name of file containing control and electrical-drive input parameters (quoted string) 
"IEA-15-255-RWT-UMaineSemi_HydroDyn.dat" HydroFile    - Name of file containing hydrodynamic input parameters (quoted string) 
              "none"   SubFile      - Name of file containing sub-structural input parameters (quoted string) 
"IEA-15-255-RWT-UMaineSemi_MoorDyn.dat" MooringFile   - Name of file containing mooring system input parameters (quoted string) 
...
--------------------------- Linearization -------------------------------------- 
             ...
             99999.9   TrimCase     - Controller parameter to be trimmed {1:yaw; 2:torque; 3:pitch} [used only if CalcSteady=True] (-)
             99999.9   TrimTol      - Tolerance for the rotational speed convergence [used only if CalcSteady=True] (-)
             99999.9   TrimGain     - Proportional gain for the rotational speed error (>0) [used only if CalcSteady=True] (rad/(rad/s) for yaw or pitch; Nm/(rad/s) for torque)
             99999.9   Twr_Kdmp     - Damping factor for the tower [used only if CalcSteady=True] (N/(m/s))
             99999.9   Bld_Kdmp     - Damping factor for the blades [used only if CalcSteady=True] (N/(m/s))
             99999.9   NLinTimes    - Number of times to linearize (-) [>=1] [unused if Linearize=False]
             99999.9   LinTimes     - List of times at which to linearize (s) [1 to NLinTimes] [used only when Linearize=True and CalcSteady=False]
             99999.9   LinInputs    - Inputs included in linearization (switch) {0=none; 1=standard; 2=all module inputs (debug)} [unused if Linearize=False]
             99999.9   LinOutputs   - Outputs included in linearization (switch) {0=none; 1=from OutList(s); 2=all module outputs (debug)} [unused if Linearize=False]
